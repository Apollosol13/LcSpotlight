...
/**
 * GET /api/cron/things-to-do-enrich — Backfill website + google_photo_name via Places API (New).
...
export async function GET(req: NextRequest) {
  ...
  try {
    const { data: rows, error: fetchErr } = await supabaseAdmin
      .from("things_to_do")
      .select(
        "id, title, venue, market_key, website, image_url, google_place_name, google_photo_name",
      )
    ...
    const candidates = list
      .filter((r) => {
        ...
        const needImage = !r.image_url?.trim() && !r.google_photo_name?.trim();
