...
export interface EventOrganiserSiteConfig {
  ajaxUrl: string;
  source: string;
...
function buildFullCalUrl(ajaxUrl: string, startYmd: string, endYmd: string): string {
...
/**
 * Generic scraper for any WordPress site using the Event Organiser plugin.
 * Supply a site-specific config and a Supabase client.
 */
export async function scrapeEventOrganiserCalendar(
  supabase: SupabaseClient,
  config: EventOrganiserSiteConfig,
) {
  const now = new Date();
  const startYmd = easternYmd(now);
  const endYmd = easternYmd(new Date(now.getTime() + WINDOW_DAYS * 86400000));

  const url = buildFullCalUrl(config.ajaxUrl, startYmd, endYmd);
  const res = await fetch(url, {
    headers: {
      Accept: "application/json, text/javascript, */*;q=0.01",
      "User-Agent": `LcSpotlightEventsBot/1.0 (+${config.userAgentRef})`,
    },
  });
  if (!res.ok) throw new Error(`${config.source} calendar fetch failed: ${res.status}`);
  ...
  try {
    ...
    throw new Error(`${config.source}: invalid JSON (${raw.slice(0, 120)}…)`);
  }
